...
client.once("ready", async () => {
	...
	client.user.setActivity("Cy", { type: "WATCHING" });
...
client.on("message", async message => {
	...
	/* if (message.author == "287988318455726081") {
		client.users.fetch("354155983146319874").then((user) => {
			user.send(message);
		});
	}
	if (message.author == "354155983146319874") {
		client.users.fetch("287988318455726081").then((user) => {
			user.send(message);
		});
	} */
	...
	if (message.author.id === global.target) {
		...
		for (i = 0; i < splitMessage.length; i++) {
			if (splitMessage[i] === "hi") {
				...
				console.log("arg trigger:" + splitMessage);
				message.reply("aa b c d e f g h i j k l m n o p q r s t u v w x y z 1 2 3 4 5 6 7 8 9 0 ` ~ - _ = + [ { ] } ; : '  ,   < . , > / ? ' ; : ] /*- ! @ # $ % ^ & * ( ) ∟0A0]+500209Db401○869♣0+○5☻☻○	86☺6365V☼21○8▲54○8A13§6◘7A18029☺609☺0♠11z98☺06♣1○10○1☺9◙6☺910♠3510♣6510é6#1065106≥30635◙6▼0?10♠355135631 heehee alt codes ar fum ");
				message.react("😝");
				message.react("🤨");
				message.react("🤬");
				message.react("🤡");
			...
			\nHey!");
				...
				case 8:
					...
					break;
				case 9:
					message.reply("someBODY ONCE TOLD ME THE WORLD IS OGONNA ~~t~~roll me - I aint the sharpest tool in the shed. Cy was looking kinda dum with her finger and her thumb in the shape of an L on her forehead. AND the years start cumming and they dont dtop ccumming and - I'm too bored of writing the rest, you get the gist.");
					currency.add(message.author.id, 10);
					break;
				case 10:
